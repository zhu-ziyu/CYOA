...
# Originally used to store the list of items purchased by the player, now all changed to global Boolean variables:
# inventory = []
...
# Original companions, offsite teams, etc. were stored in offsite_team and associates, now changed to global boolean
# offsite_team = []
...
# 是否已经使用过某些同伴的技能.
# Whether some companion's skills have been used.
...
# Play audio (only used for vault heist countdown) has been disabled...................
...
#Random function after entering the fighting scene....
def random_event():
...
#Random function after entering the fighting scene....#This is a fixed viewing function that I have no idea how to write..
...
#这个功能并未完成...后续可能补齐
#This function is not yet complete...it may be completed later
...
def equipment_shop():
    ...
    #Now use simple print + if/elif to let players enter purchase
    print_divider()
    ...
    item_bought = False
    #由于这部分并没有太多用户选项，仅通过金钱数量判断玩家金钱是否足够
    # Since there are not many user options in this part, only the amount of money is used to determine whether the player has enough money
...
def recruitment_associates():
    ...
    print("Associate: SAM")
    print("Share: 0%")
    print("Description: Test mode: doubles your HP and damage.(Test using a character. Using this character may make the game too easy. . . .)")
    print_divider()
...
# 选择接近银行方式
# Choose the method to approach the bank
...
def main_entrance_after_combat():
    ...
    print("'Hostage Control DLC(ENTER HOSTAGE)'       - Control the hostages and negotiate with the police for a ransom (but subsequent escape will be more difficult) (DLC may not be completed yet/is in production,this DLC is very large!!!!!!!)")
    print_divider()
...
def hostage_control():
    print("I submitted two files, just replace this line with the files I submitted to play the dlc.")
    main_entrance_after_combat()
